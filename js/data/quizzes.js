...
export const QUIZZES = {
    ...
    6: {
        ...
        questions: [
            {
                question: 'Q1 — In Level 6, you wrote `class MyNinja extends NinjaBot`. What OOP principle allows your ninja to automatically know how to run and jump without you writing those methods?',
                options: [
                    'A) Encapsulation',
                    'B) Inheritance',
                    'C) Method Overloading',
                ...
                correct: 1,
                explanation: 'Inheritance allows a child class (`MyNinja`) to inherit fields and methods from a parent class (`NinjaBot`). You get access to `this.run()` and `this.jump()` for free!',
            },
            {
                question: 'Q2 — You defined `decideAction(state)` inside `MyNinja`. The base `NinjaBot` already had a `decideAction` method, but yours was executed instead. What is this called?',
                options: [
                    'A) Method Overriding',
                ...
                correct: 0,
                explanation: 'Method Overriding occurs when a child class provides a specific implementation of a method that is already provided by its parent class. This is a form of runtime polymorphism.',
            },
            {
                question: 'Q3 — You used `state.hasEnemy()` instead of checking a raw variable like `state.enemyAhead == true`. Why is using a method better?',
                options: [
                    'A) It runs faster in JavaScript.',
                    'B) It hides the internal logic of how an enemy is detected, protecting the state (Encapsulation).',
                    'C) It forces the use of inheritance.',
                    'D) It allows you to create new objects instantly.',
                ],
                correct: 1,
                explanation: 'Encapsulation hides the internal details of an object. By using a method like `hasEnemy()`, the `state` object can change how it detects enemies internally without breaking your ninja code.',
            },
            {
                question: 'Q4 — You used a "Guard Clause" to check for danger first (`if (state.hasEnemy()) return this.hide();`). What is the primary benefit of Guard Clauses?',
                options: [
                ...
                correct: 1,
                explanation: 'Guard clauses are an early-exit strategy. By handling specific conditions at the top of a function, you avoid nesting your primary logic deep inside `else` blocks.',
            },
            {
                question: 'Q5 — Why did you use `this.hide()` instead of just `hide()` or `NinjaBot.hide()`?',
                options: [
                    'A) `this` refers to the specific instance of the object executing the method.',
                    'B) `this` creates a static method.',
                    'C) `this` prevents other classes from using the method.',
                    'D) `this` is required by all JavaScript loops.',
                ],
                correct: 0,
                explanation: 'The `this` keyword refers to the current instance of the class. Since `hide()` is an instance method inherited from `NinjaBot`, you must call it on the current object instance using `this`.',
            },
            {
                ...
                correct: 1,
                explanation: 'The Arrow Anti-Pattern happens when nested `if/else` blocks indent the code so far to the right it looks like a wedge. Guard clauses fix this by keeping code flat and linear.',
            },
            {
                question: 'Q7 — When `this.hide()` is called, the specific animation and collision logic is hidden inside the Engine, while you just call a simple method. This demonstrates:',
                options: [
                    'A) Abstraction',
                    'B) Inheritance',
                    'C) Polymorphism',
                ...
                correct: 0,
                explanation: 'Abstraction hides complex backend implementation details (how the canvas renders hiding) behind a simple, high-level interface (`this.hide()`).',
            },
            {
                question: 'Q8 — You were required to include `return this.run();` at the very end. Why is this important?',
                options: [
                    'A) To provide a "Default Fallback" action in case no specific `if` conditions are met.',
                    'B) Because JavaScript requires every method to return a string.',
                    'C) To override the parent class method.',
                    'D) To create an infinite loop.',
                ],
                correct: 0,
                explanation: 'A default return acts as a safety net. If the Ninja isn\'t facing an obstacle or an enemy, the method falls through the `if` statements and safely returns the default "RUN" action.',
            },
            {
                question: 'Q9 — The engine validates your code by checking if it contains `class MyNinja extends NinjaBot`. Why might a static analysis tool do this in the real world?',
                options: [
                    'A) To prevent infinite loops.',
                    'B) To enforce coding standards and ensure the correct architectural patterns are being used.',
                    'C) To compile JavaScript into C++.',
                    'D) To minify the file size.',
                ],
                correct: 1,
                explanation: 'Static analysis tools (like ESLint) scan code before it runs to enforce best practices and ensure developers follow the required structural guidelines.',
            },
            {
                question: 'Q10 — The `decideAction` method only handles decision making. It does not update the Ninja\'s x/y coordinates or draw the canvas. This is a perfect example of:',
                options: [
